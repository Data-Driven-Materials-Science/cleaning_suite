...
data_df = pd.read_csv("../datasets/Results_FourDimensional_Datafile.csv")
...
for col in data_df.columns:
    ...
    plt.plot(data_df[col], color=ck.raw_data_color)
    plt.title("Raw Data Values - " + str(col))
    plt.xlabel("Point Index")
    plt.ylabel(str(col))
    plt.savefig("./paper_images/results_4d_time_series_before_" + str(col) + ".png", dpi=500)
    plt.show()
